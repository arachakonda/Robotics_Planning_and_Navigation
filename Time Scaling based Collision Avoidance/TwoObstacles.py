...
#                      [ Bc1dot, Bc2dot, Bc3dot, Bc4dot ] ] )
    W = np.linalg.inv( Bx.T@Bx + np.eye(4)*0.0000002 )@Bx.T@Ax   #np.linalg.inv( Bx)@Ax
...
#     print( k0, kf , yt0,ytf ,kc, ytc )
    
#     k0, kf , yt0,ytf ,kc, ytc = 0 ,0,0,40,1,40
    
#     print( k0, kf , yt0,ytf ,kc, ytc )
    
#     print( Bc0,Bc1,Bc2,Bc3,Bc4,Bc5 )
    
    BC = np.array( [ Bc0,Bc1,Bc2,Bc3,Bc4,Bc5 ])
...
#                    [ Bc1,Bc2,Bc3,Bc4  ]])
    ...
    W = np.linalg.inv(Bk.T@Bk  + np.eye(4)*0.000000002  )@(Bk.T)@Ak  # + np.eye(4)*0.000002
    
    
    
    Wk = np.asarray([ Wk0 , W[0][0] , W[1][0] , W[2][0] , W[3][0] , Wkf ]) 
...
def GetDirection(theta  ):
    ...
    for i in range( length ):
        ...
        x_dir = np.cos(theta[i])
        y_dir = np.sin(theta[i])
        x_direction.append( x_dir )
...
def GetScale( CurPoseEgo , CurPoseObs, CollisionPt , delt , Obsradius ):

    CurPoseEgo = np.asarray(CurPoseEgo)
    CurPoseObs  =np.asarray(CurPoseObs)

    EgoVel = (CollisionPt -CurPoseEgo )/delt
#     EgoVel /= np.linalg.norm(EgoVel)
    ObsVel = (CollisionPt - CurPoseObs )/delt 
#     ObsVel /= np.linalg.norm(ObsVel)
    RelativeVel = (EgoVel -ObsVel  )
    
    RelativeVelMag = np.linalg.norm(RelativeVel)
    
    r = CurPoseEgo - CurPoseObs
    
    K1 = (np.linalg.norm( r ) )**2 -Obsradius**2
    
    k2x = EgoVel[0]*CurPoseEgo[0]
    k3x = EgoVel[0]*CurPoseObs[0]
    k4x = ObsVel[0]*CurPoseEgo[0]
    k5x = ObsVel[0]*CurPoseObs[0]
    
    k2y = EgoVel[1]*CurPoseEgo[1]
    k3y = EgoVel[1]*CurPoseObs[1]
    k4y = ObsVel[1]*CurPoseEgo[1]
    k5y = ObsVel[1]*CurPoseObs[1]
    
    K2 = k2x - k3x + k2y - k3y
    K3 = - k4x + k5x - k4y + k5y 
    
    k1D = np.linalg.norm( EgoVel)**2
    k2D = EgoVel[0]*ObsVel[0] + EgoVel[1]*ObsVel[1]
    k3D =  np.linalg.norm( ObsVel)**2
    ...
    if( a < 0 ):
        ...
        s =  ( -b +np.sqrt(  np.abs( np.around(b , 0)**2 - 4*np.around(a ,0)*np.around(c , 0) ) )) /(2* np.abs(a)) -0.2
...
def PathRobot1():
    ...
    tf1,tw1,start1,waypt1,goal1,init_or1,waypt_or1,final_or1,dx_01,dx_f1,dx_w1 = 5,2.5,[0,0],[5,5],[10,10],0,np.pi/6, np.pi/4 ,0,0,0

    tf2,tw2,start2,waypt2,goal2,init_or2,waypt_or2,final_or2,dx_02,dx_f2,dx_w2 = 5,2.5,[10,0],[7.5,7.5],[0,10],0,np.pi/6, np.pi/4 ,0,0,0 


    Wx1 = DetermineBernsteinCoeffsX( start1[0] , goal1[0] , waypt1[0]  , dx_01 , dx_f1 ,  dx_w1 , 0 , tw1  ,  tf1 )

    Wx01,Wx11,Wx21,Wx31,Wx41,Wx51 = Wx1[0] , Wx1[1] , Wx1[2] , Wx1[3] , Wx1[4] , Wx1[5] 

    Wx2 = DetermineBernsteinCoeffsX( start2[0] , goal2[0] , waypt2[0]  , dx_02 , dx_f2 ,  dx_w2 , 0 , tw2  ,  tf2 )

    Wx02,Wx12,Wx22,Wx32,Wx42,Wx52 = Wx2[0] , Wx2[1] , Wx2[2] , Wx2[3] , Wx2[4] , Wx2[5] 


    Wk1 = DetermineBernsteinCoeffsK( waypt_or1 , final_or1 , waypt_or1  ,  dx_w1  , dx_w1 , 
                                       start1[1] , waypt1[1]  , goal1[1] , 0 , tf1 , Wx1 , tw1 )
  

    Wk2 = DetermineBernsteinCoeffsK( waypt_or2 , final_or2 , waypt_or2  ,  dx_w2  , dx_w2 , 
                                       start2[1] , waypt2[1]  , goal2[1] , 0 , tf2 , Wx2 , tw2 )
    ...
    while t <  tf1 :
        ...
        B0,B1,B2,B3,B4,B5 = get_bernstein_poly(0,t,tf)
        ...
        coef0t1, coef1t1, coef2t1, coef3t1, coef4t1, coef5t1 = get_product_functions( Wx01,Wx11,Wx21,Wx31,Wx41,Wx51 ,0,t,tf )

        coef0t2, coef1t2, coef2t2, coef3t2, coef4t2, coef5t2 = get_product_functions( Wx02,Wx12,Wx22,Wx32,Wx42,Wx52 ,0,t,tf )


        F1 = np.array( [ coef0t1, coef1t1, coef2t1, coef3t1, coef4t1, coef5t1  ])
        F2 = np.array( [ coef0t2, coef1t2, coef2t2, coef3t2, coef4t2, coef5t2  ])

        ypt1 = start1[0] + Wk1@F1
        ypt2 = start2[0] + Wk2@F2  -10

        Robot1Pose = np.array([xpt1 , ypt1])
        Robot2Pose = np.array([xpt2 , ypt2])

        distance = np.linalg.norm( Robot1Pose - Robot2Pose )

        

        if(  distance < 1 ):

            scale = GetScale(  [ X1[-1] , Y1[-1]  ] , [ X2[-1] , Y2[-1]  ] , [ xpt2 , ypt2 ] , delt ,  Obsradius )
            ...
            B01,B11,B21,B31,B41,B51 = get_bernstein_poly(0,t,tf)
            B02,B12,B22,B32,B42,B52 = get_bernstein_poly(0,t2,tf)

            B1 = np.array( [ B01,B11,B21,B31,B41,B51 ])
            B2 = np.array( [ B02,B12,B22,B32,B42,B52 ])

            coef0t1, coef1t1, coef2t1, coef3t1, coef4t1, coef5t1 = get_product_functions( Wx01,Wx11,Wx21,Wx31,Wx41,Wx51,0,t,tf )
            coef0t2, coef1t2, coef2t2, coef3t2, coef4t2, coef5t2 = get_product_functions( Wx02,Wx12,Wx22,Wx32,Wx42,Wx52 ,0,t2,tf )
